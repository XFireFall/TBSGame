...
std::ostream& gm::operator<<(std::ostream& os, gm::Phase const& ph)
{
	std::string res;
	switch (ph) {
		case gm::WAIT:   res = "<wait>";   break;
		case gm::ATTACK: res = "<attack>"; break;
		case gm::FEED:   res = "<feed>";   break;
		case gm::WIN:    res = "<win>";    break;
		case gm::LOSE:   res = "<lose>";   break;
		default:         res = "<? ? ?>";  break;
	}
	return os << res;
}
...
bool Field::belongsTo(uint16_t owner) const
{
	for (auto ln: *this)
		for (auto cell: ln)
			if (!cell.belongsTo(owner))
